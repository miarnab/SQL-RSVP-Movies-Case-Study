...
FROM imdb.movie left join 
imdb.ratings on movie .id = ratings.movie_id
...
SELECT   m.production_company AS production_company, COUNT(*) AS movie_count,
...
WHERE genre IS NOT NULL AND date_published BETWEEN '2017-03-01' AND '2017-03-31' AND total_votes > 1000
GROUP BY genre;
...
FROM movie m JOIN genre g ON m.id = g.movie_id JOIN ratings r ON m.id = r.movie_id
...
SELECT nm.name , COUNT(*) AS movie_count
FROM movie m 
JOIN ratings r ON m.id = r.movie_id
JOIN genre g ON m.id = g.movie_id
JOIN director_mapping d ON m.id = d.movie_id
JOIN names nm ON d.name_id = nm.id
WHERE r.avg_rating > 8
  AND g.genre IN (SELECT  g.genre
FROM movie m 
JOIN ratings r ON m.id = r.movie_id
JOIN genre g ON m.id = g.movie_id
WHERE r.avg_rating > 8
